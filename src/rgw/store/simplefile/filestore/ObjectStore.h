...
class ObjectStore {
 ...
 public:
   ...
   */
  static std::unique_ptr<ObjectStore> create(CephContext* cct,
   ...
   */
  static int probe_block_device_fsid(CephContext* cct, const std::string& path,
                                     uuid_d* fsid);
   ...
   */
  virtual rgw_filestore_perf_stat_t get_cur_stats() = 0;
  ...
  struct CollectionImpl : public RefCountedObject {
    ...
    const rgw_salcoll_t& get_cid() {
      return cid;
    }

   protected:
    CollectionImpl() = delete;
    CollectionImpl(CephContext* cct, const rgw_salcoll_t& c)
   ...
   * (rgw_saloid_t and hobject_t) in a named collection (rgw_salcoll_t).
   ...
   * names (rgw_salcoll_t) and can be enumerated in order.  Like an
   * individual object, a collection also has a std::set of xattrs.
   *
   *
   */

  int queue_transaction(CollectionHandle& ch, Transaction&& t,
  ...
  virtual int queue_transactions(CollectionHandle& ch,
                                 std::vector<Transaction>& tls,
                                 TrackedOpRef op = TrackedOpRef(),
 ...
 public:
  ...
  virtual int flush_cache(std::ostream* os = NULL) {
    return -1;
  }
  ...
  virtual int get_numa_node(int* numa_node, std::set<int>* nodes,
  ...
  virtual int statfs(struct rgw_filestore_statfs_t* buf,
   ...
   * implementations.
   */
  virtual CollectionHandle open_collection(const rgw_salcoll_t& cid) = 0;
   ...
   */
  virtual CollectionHandle create_new_collection(const rgw_salcoll_t& cid) = 0;
   ...
   */
  virtual void set_collection_commit_queue(const rgw_salcoll_t& cid,
   ...
   */
  virtual bool exists(CollectionHandle& c, const rgw_saloid_t& oid) = 0;
   ...
   */
  virtual int stat(CollectionHandle& c, const rgw_saloid_t& oid,
                   struct stat* st, bool allow_eio = false) = 0;
  /**
   ...
   */
  virtual int read(CollectionHandle& c, const rgw_saloid_t& oid,
   ...
   * @returns 0 on success, negative error code on failure.
   */
  virtual int fiemap(CollectionHandle& c, const rgw_saloid_t& oid,
   ...
   * @returns number of bytes read on success, or negative error code on failure.
   */
  virtual int readv(CollectionHandle& c, const rgw_saloid_t& oid,
   ...
   * @returns 0 on success, negative error code on failure.
   */
  virtual int list_collections(std::vector<rgw_salcoll_t>& ls) = 0;
   ...
   */
  virtual bool collection_exists(const rgw_salcoll_t& c) = 0;
  ...
  /**
   * return the number of significant bits of the rgw_salcoll_t::pgid.
   *
  ...
  virtual int dump_journal(std::ostream& out) {
    return -EOPNOTSUPP;
  }

  virtual int snapshot(const std::string& name) {
    return -EOPNOTSUPP;
  }
  ...
  */
  ...
  // DEBUG
  virtual void inject_data_error(const rgw_saloid_t& oid) {
  }
  virtual void inject_mdata_error(const rgw_saloid_t& oid) {
  }
