...
int JournalingObjectStore::journal_replay(uint64_t fs_op_seq) {
  ...
  if (cct->_conf->journal_replay_from) {
    dout(0) << "journal_replay forcing replay from "
            << cct->_conf->journal_replay_from << " instead of " << fs_op_seq
            << dendl;
  ...
  while (1) {
    ...
    if (seq <= op_seq) {
      dout(3) << "journal_replay: skipping old op seq " << seq
              << " <= " << op_seq << dendl;
      continue;
    }
    ...
    dout(3) << "journal_replay: r = " << r << ", op_seq now " << op_seq
            << dendl;
  }

  if (count) dout(3) << "journal_replay: total = " << count << dendl;
  ...
  if (!count) journal->committed_thru(fs_op_seq);
...
void JournalingObjectStore::ApplyManager::op_apply_finish(uint64_t op) {
  ...
  if (op > max_applied_seq) max_applied_seq = op;
}
...
bool JournalingObjectStore::ApplyManager::commit_start() {
  ...
  if (journal) journal->commit_start(committing_seq);  // tell the journal too
out:
  return ret;
}
...
void JournalingObjectStore::ApplyManager::commit_finish() {
  ...
  if (journal) journal->committed_thru(committing_seq);

  committed_seq = committing_seq;

  map<version_t, vector<Context *> >::iterator p = commit_waiters.begin();
  while (p != commit_waiters.end() && p->first <= committing_seq) {
    finisher.queue(p->second);
    commit_waiters.erase(p++);
  }
}

void JournalingObjectStore::_op_journal_transactions(bufferlist &tbl,
                                                     uint32_t orig_len,
                                                     uint64_t op,
                                                     Context *onjournal,
                                                     TrackedOpRef osd_op) {
  if (osd_op.get()) {
    // TODO (OpRequest reference)
