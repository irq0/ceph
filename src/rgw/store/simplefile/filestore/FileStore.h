...
class FileStore : public JournalingObjectStore, public md_config_obs_t {
 ...
 public:
  ...
  struct FSPerfTracker {
    ...
    rgw_filestore_perf_stat_t get_cur_stats() const {
      rgw_filestore_perf_stat_t ret;
  ...
  rgw_filestore_perf_stat_t get_cur_stats() override {
 ...
 private:
  std::string
      internal_name;  ///< internal name, used to name the perfcounter instance
  ...
  int get_index(const rgw_salcoll_t& c, Index* index);
  int init_index(const rgw_salcoll_t& c);

  void handle_eio();

  // helper fns
  int get_cdir(const rgw_salcoll_t& cid, char* s, int len);
  ...
  ceph::mutex sync_entry_timeo_lock =
  ...
  class OpSequencer : public CollectionImpl {
    ...
    /// hash of pointers to rgw_saloid_t's for in-flight writes
    std::unordered_multimap<uint32_t, const rgw_saloid_t*> applying;
   ...
   public:
    ...
    /// get_max_uncompleted
    bool _get_max_uncompleted(uint64_t* seq  ///< [out] max uncompleted seq
    ) {
      ceph_assert(seq);
      *seq = 0;
      if (q.empty() && jq.empty()) return true;

      if (!q.empty()) *seq = q.back()->op;
      if (!jq.empty() && jq.back() > *seq) *seq = jq.back();

      return false;
    }  /// @returns true if both queues are empty

    /// get_min_uncompleted
    bool _get_min_uncompleted(uint64_t* seq  ///< [out] min uncompleted seq
    ) {
      ceph_assert(seq);
      *seq = 0;
      if (q.empty() && jq.empty()) return true;

      if (!q.empty()) *seq = q.front()->op;
      if (!jq.empty() && jq.front() < *seq) *seq = jq.front();
    ...
    void _wake_flush_waiters(std::list<Context*>* to_queue) {
      uint64_t seq;
      if (_get_min_uncompleted(&seq)) seq = -1;
    ...
    void wait_for_apply(const rgw_saloid_t& oid);
    Op* peek_queue() {
      std::lock_guard l{qlock};
   ...
   private:
    ...
    OpSequencer(CephContext* cct, int i, rgw_salcoll_t cid)
  ...
  std::map<rgw_salcoll_t, OpSequencerRef> coll_map;
  ...
  struct OpWQ : public ThreadPool::WorkQueue<OpSequencer> {
    FileStore* store;
    OpWQ(FileStore* fs, ceph::timespan timeout, ceph::timespan suicide_timeout,
         ThreadPool* tp)
    ...
    OpSequencer* _dequeue() override {
      if (store->op_queue.empty()) return nullptr;
  ...
  Op* build_op(std::vector<Transaction>& tls, Context* onreadable,
 ...
 public:
  int lfn_find(const rgw_saloid_t& oid, const Index& index,
               IndexedPath* path = nullptr);
  int lfn_truncate(const rgw_salcoll_t& cid, const rgw_saloid_t& oid,
                   off_t length);
  int lfn_stat(const rgw_salcoll_t& cid, const rgw_saloid_t& oid,
               struct stat* buf);
  int lfn_open(const rgw_salcoll_t& cid, const rgw_saloid_t& oid, bool create,
               FDRef* outfd, Index* index = nullptr);

  void lfn_close(FDRef fd);
  int lfn_link(const rgw_salcoll_t& c, const rgw_salcoll_t& newcid,
               const rgw_saloid_t& o, const rgw_saloid_t& newoid);
  int lfn_unlink(const rgw_salcoll_t& cid, const rgw_saloid_t& o,
                 const SequencerPosition& spos);

 public:
  FileStore(CephContext* cct, const std::string& base, const std::string& jdev,
            osflagbits_t flags = 0, const char* internal_name = "filestore",
  ...
  int statfs(struct rgw_filestore_statfs_t* buf,
             osd_alert_list_t* alerts = nullptr) override;

  int _do_transactions(std::vector<Transaction>& tls, uint64_t op_seq,
  ...
  CollectionHandle open_collection(const rgw_salcoll_t& c) override;
  CollectionHandle create_new_collection(const rgw_salcoll_t& c) override;
  void set_collection_commit_queue(const rgw_salcoll_t& cid,
  ...
  void _set_replay_guard(int fd, const SequencerPosition& spos,
                         const rgw_saloid_t* oid = 0, bool in_progress = false);
  void _set_replay_guard(const rgw_salcoll_t& cid,
                         const SequencerPosition& spos, bool in_progress);
  void _set_global_replay_guard(const rgw_salcoll_t& cid,
                                const SequencerPosition& spos);

  /// close a replay guard opened with in_progress=true
  void _close_replay_guard(int fd, const SequencerPosition& spos,
                           const rgw_saloid_t* oid = 0);
  void _close_replay_guard(const rgw_salcoll_t& cid,
                           const SequencerPosition& spos);
  ...
  int _check_replay_guard(const rgw_salcoll_t& cid,
                          const SequencerPosition& spos);
  int _check_replay_guard(const rgw_salcoll_t& cid, const rgw_saloid_t& oid,
                          const SequencerPosition& pos);
  int _check_global_replay_guard(const rgw_salcoll_t& cid,
                                 const SequencerPosition& spos);

  // ------------------
  // objects
  int pick_object_revision_lt(rgw_saloid_t& oid) {
    return 0;
  }
  using ObjectStore::exists;
  bool exists(CollectionHandle& c, const rgw_saloid_t& oid) override;
  using ObjectStore::stat;
  int stat(CollectionHandle& c, const rgw_saloid_t& oid, struct stat* st,
           bool allow_eio = false) override;
  using ObjectStore::read;
  int read(CollectionHandle& c, const rgw_saloid_t& oid, uint64_t offset,
  ...
  int fiemap(CollectionHandle& c, const rgw_saloid_t& oid, uint64_t offset,
             size_t len, ceph::buffer::list& bl) override;
  int fiemap(CollectionHandle& c, const rgw_saloid_t& oid, uint64_t offset,
             size_t len, std::map<uint64_t, uint64_t>& destmap) override;

  int _touch(const rgw_salcoll_t& cid, const rgw_saloid_t& oid);
  int _write(const rgw_salcoll_t& cid, const rgw_saloid_t& oid, uint64_t offset,
             size_t len, const ceph::buffer::list& bl,
             uint32_t fadvise_flags = 0);
  int _zero(const rgw_salcoll_t& cid, const rgw_saloid_t& oid, uint64_t offset,
            size_t len);
  int _truncate(const rgw_salcoll_t& cid, const rgw_saloid_t& oid,
                uint64_t size);
  int _clone(const rgw_salcoll_t& cid, const rgw_saloid_t& oldoid,
             const rgw_saloid_t& newoid, const SequencerPosition& spos);
  int _clone_range(const rgw_salcoll_t& oldcid, const rgw_saloid_t& oldoid,
                   const rgw_salcoll_t& newcid, const rgw_saloid_t& newoid,
                   uint64_t srcoff, uint64_t len, uint64_t dstoff,
                   const SequencerPosition& spos);
  int _do_clone_range(int from, int to, uint64_t srcoff, uint64_t len,
                      uint64_t dstoff);
  int _do_sparse_copy_range(int from, int to, uint64_t srcoff, uint64_t len,
                            uint64_t dstoff);
  int _do_copy_range(int from, int to, uint64_t srcoff, uint64_t len,
                     uint64_t dstoff, bool skip_sloppycrc = false);
  int _remove(const rgw_salcoll_t& cid, const rgw_saloid_t& oid,
              const SequencerPosition& spos);

  int _fgetattr(int fd, const char* name, ceph::bufferptr& bp);
  int _fgetattrs(int fd,
                 std::map<std::string, ceph::bufferptr, std::less<>>& aset);
  int _fsetattrs(int fd,
                 std::map<std::string, ceph::bufferptr, std::less<>>& aset);
  ...
  std::set<rgw_saloid_t> data_error_set;   // read() will return -EIO
  std::set<rgw_saloid_t> mdata_error_set;  // getattr(),stat() will return -EIO
  void inject_data_error(const rgw_saloid_t& oid) override;
  void inject_mdata_error(const rgw_saloid_t& oid) override;
  ...
  void debug_obj_on_delete(const rgw_saloid_t& oid);
  bool debug_data_eio(const rgw_saloid_t& oid);
  bool debug_mdata_eio(const rgw_saloid_t& oid);

  int snapshot(const std::string& name) override;

  int _collection_remove_recursive(const rgw_salcoll_t& cid,
                                   const SequencerPosition& spos);

  int _collection_set_bits(const rgw_salcoll_t& cid, int bits);

  // collections
  using ObjectStore::collection_list;
  int collection_bits(CollectionHandle& c) override;
  int collection_list(CollectionHandle& c, const rgw_saloid_t& start,
  ...
  int collection_list(const rgw_salcoll_t& cid, const rgw_saloid_t& start,
                      const rgw_saloid_t& end, int max,
                      std::vector<rgw_saloid_t>* ls, rgw_saloid_t* next);
  int list_collections(std::vector<rgw_salcoll_t>& ls) override;
  int collection_stat(const rgw_salcoll_t& c, struct stat* st);
  bool collection_exists(const rgw_salcoll_t& c) override;
  int collection_empty(CollectionHandle& c, bool* empty) override {
    c->flush();
    return collection_empty(c->cid, empty);
  }
  int collection_empty(const rgw_salcoll_t& cid, bool* empty);

  int _create_collection(const rgw_salcoll_t& c, int bits,
                         const SequencerPosition& spos);
  int _destroy_collection(const rgw_salcoll_t& c);
  /**
   ...
   */
  int _collection_hint_expected_num_objs(const rgw_salcoll_t& c,
                                         uint64_t expected_num_objs,
                                         const SequencerPosition& spos);
  int _collection_add(const rgw_salcoll_t& c, const rgw_salcoll_t& ocid,
                      const rgw_saloid_t& oid, const SequencerPosition& spos);
  int _collection_move_rename(const rgw_salcoll_t& oldcid,
                              const rgw_saloid_t& oldoid, rgw_salcoll_t c,
                              const rgw_saloid_t& o,
                              const SequencerPosition& spos,
                              bool ignore_enoent = false);

  int _set_alloc_hint(const rgw_salcoll_t& cid, const rgw_saloid_t& oid,
                      uint64_t expected_object_size,
                      uint64_t expected_write_size);

  void dump_start(const std::string& file);
  void dump_stop();
  void dump_transactions(std::vector<Transaction>& ls, uint64_t seq,
                         OpSequencer* osr);

  virtual int apply_layout_settings(const rgw_salcoll_t& cid, int target_level);

 private:
  void _inject_failure();

  int _split_collection(const rgw_salcoll_t& cid, uint32_t bits, uint32_t rem,
                        rgw_salcoll_t dest, const SequencerPosition& spos);
  int _merge_collection(const rgw_salcoll_t& cid, uint32_t bits,
                        rgw_salcoll_t dest, const SequencerPosition& spos);
...
class FileStoreBackend {
 ...
 protected:
  ...
  int _copy_range(int from, int to, uint64_t srcoff, uint64_t len,
                  uint64_t dstoff) {
 ...
 public:
  ...
  virtual int do_fiemap(int fd, off_t start, size_t len,
                        struct fiemap** pfiemap) = 0;
  virtual int clone_range(int from, int to, uint64_t srcoff, uint64_t len,
                          uint64_t dstoff) = 0;
  virtual int set_alloc_hint(int fd, uint64_t hint) = 0;
  virtual bool has_splice() const = 0;

  // hooks for (sloppy) crc tracking
  virtual int _crc_update_write(int fd, loff_t off, size_t len,
                                const ceph::buffer::list& bl) = 0;
  virtual int _crc_update_truncate(int fd, loff_t off) = 0;
  virtual int _crc_update_zero(int fd, loff_t off, size_t len) = 0;
  virtual int _crc_update_clone_range(int srcfd, int destfd, loff_t srcoff,
                                      size_t len, loff_t dstoff) = 0;
  virtual int _crc_verify_read(int fd, loff_t off, size_t len,
