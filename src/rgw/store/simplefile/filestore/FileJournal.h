 ...
 */
class FileJournal : public Journal, public md_config_obs_t {
 public:
  ...
  int prepare_entry(std::vector<ObjectStore::Transaction> &tls,
                    ceph::buffer::list *tbl) override;
  ...
  struct entry_header_t {
    ...
    uint32_t
        crc32c;  // payload only.  not header, pre_pad, post_pad, or footer.
    uint32_t len;
    ...
    bool check_magic(off64_t pos, uint64_t fsid) {
      return magic1 == (uint64_t)pos && magic2 == (fsid ^ seq ^ len);
    }
 ...
 private:
  ...
  // in journal
  std::deque<std::pair<uint64_t, off64_t> >
      journalq;  // track seq offsets, so we can trim later.
  ...
  int prepare_multi_write(ceph::buffer::list &bl, uint64_t &orig_ops,
                          uint64_t &orig_bytee);
  int prepare_single_write(write_item &next_write, ceph::buffer::list &bl,
 ...
 public:
  FileJournal(CephContext *cct, uuid_d fsid, Finisher *fin,
              ceph::condition_variable *sync_cond, const char *f,
    ...
    if (aio && !directio) {
      lderr(cct) << "FileJournal::_open_any: aio not supported without "
                    "directio; disabling aio"
                 << dendl;
      aio = false;
    }
#ifndef HAVE_LIBAIO
    if (aio && ::getenv("CEPH_DEV") == NULL) {
      lderr(cct)
          << "FileJournal::_open_any: libaio not compiled in; disabling aio"
          << dendl;
      aio = false;
    }
#endif
  ...
  enum read_entry_result { SUCCESS, FAILURE, MAYBE_CORRUPT };
  ...
  bool read_entry(ceph::buffer::list &bl, uint64_t &last_seq, bool *corrupt);

  bool read_entry(ceph::buffer::list &bl, uint64_t &last_seq) override {
    return read_entry(bl, last_seq, 0);
  }

  // Debug/Testing
  void get_header(uint64_t wanted_seq, off64_t *_pos, entry_header_t *h);
  void corrupt(int wfd, off64_t corrupt_at);
  void corrupt_payload(int wfd, uint64_t seq);
  void corrupt_footer_magic(int wfd, uint64_t seq);
  void corrupt_header_magic(int wfd, uint64_t seq);
};
