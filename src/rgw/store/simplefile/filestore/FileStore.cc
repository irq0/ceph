...
void FileStore::FSPerfTracker::update_from_perfcounters(PerfCounters &logger) {
  os_commit_latency_ns.consume_next(
      logger.get_tavg_ns(l_filestore_journal_latency));
  os_apply_latency_ns.consume_next(
      logger.get_tavg_ns(l_filestore_apply_latency));
}
...
int FileStore::get_cdir(const rgw_salcoll_t &cid, char *s, int len) {
...
int FileStore::get_index(const rgw_salcoll_t &cid, Index *index) {
  int r = index_manager.get_index(cid, basedir, index);
  if (r == -EIO && m_filestore_fail_eio) handle_eio();
  return r;
}

int FileStore::init_index(const rgw_salcoll_t &cid) {
  char path[PATH_MAX];
...
int FileStore::lfn_find(const rgw_saloid_t &oid, const Index &index,
                        IndexedPath *path) {
  IndexedPath path2;
  if (!path) path = &path2;
  int r, exist;
  ...
  if (!exist) return -ENOENT;
  return 0;
}

int FileStore::lfn_truncate(const rgw_salcoll_t &cid, const rgw_saloid_t &oid,
                            off_t length) {
  FDRef fd;
...
int FileStore::lfn_stat(const rgw_salcoll_t &cid, const rgw_saloid_t &oid,
                        struct stat *buf) {
  IndexedPath path;
  Index index;
...
int FileStore::lfn_open(const rgw_salcoll_t &cid, const rgw_saloid_t &oid,
                        bool create, FDRef *outfd, Index *index) {
  ceph_assert(outfd);
  int r = 0;
  bool need_lock = true;
  int flags = O_RDWR;
  ...
  if (r < 0) {
    derr << "could not find " << oid << " in index: " << cpp_strerror(-r)
         << dendl;
    goto fail;
  }
  ...
  if (r < 0) {
    r = -errno;
    dout(10) << "error opening file " << (*path)->path()
             << " with flags=" << flags << ": " << cpp_strerror(-r) << dendl;
    goto fail;
  }
  fd = r;
  if (create && (!exist)) {
...
int FileStore::lfn_link(const rgw_salcoll_t &c, const rgw_salcoll_t &newcid,
                        const rgw_saloid_t &o, const rgw_saloid_t &newoid) {
...
int FileStore::lfn_unlink(const rgw_salcoll_t &cid, const rgw_saloid_t &o,
  ...
  r = index->unlink(o);
  if (r < 0) {
    dout(25) << __FUNC__ << ": index unlink failed " << cpp_strerror(r)
             << dendl;
    return r;
  }
  return 0;
...
FileStore::FileStore(CephContext *cct, const std::string &base,
    ...
    : JournalingObjectStore(cct, base),
      ...
      throttle_ops(cct, "filestore_ops",
                   cct->_conf->filestore_caller_concurrency),
      ...
      op_tp(cct, "FileStore::op_tp", "tp_fstore_op",
            cct->_conf->filestore_op_threads, "filestore_op_threads"),
      op_wq(
          this, ceph::make_timespan(cct->_conf->filestore_op_thread_timeout),
      ...
      m_filestore_sloppy_crc_block_size(
          cct->_conf->filestore_sloppy_crc_block_size),
      m_filestore_max_alloc_hint_size(
          cct->_conf->filestore_max_alloc_hint_size),
      m_fs_type(0) {
  ...
  // initialize logger
  PerfCountersBuilder plb(cct, internal_name, l_filestore_first,
                          l_filestore_last);

  plb.add_u64(l_filestore_journal_queue_ops, "journal_queue_ops",
              "Operations in journal queue");
  plb.add_u64(l_filestore_journal_ops, "journal_ops",
              "Active journal entries to be applied");
  plb.add_u64(l_filestore_journal_queue_bytes, "journal_queue_bytes",
              "Size of journal queue");
  plb.add_u64(l_filestore_journal_bytes, "journal_bytes",
              "Active journal operation size to be applied");
  plb.add_time_avg(l_filestore_journal_latency, "journal_latency",
                   "Average journal queue completing latency", NULL,
                   PerfCountersBuilder::PRIO_USEFUL);
  plb.add_u64_counter(l_filestore_journal_wr, "journal_wr",
                      "Journal write IOs");
  plb.add_u64_avg(l_filestore_journal_wr_bytes, "journal_wr_bytes",
                  "Journal data written");
  plb.add_u64(l_filestore_op_queue_max_ops, "op_queue_max_ops",
              "Max operations in writing to FS queue");
  plb.add_u64(l_filestore_op_queue_ops, "op_queue_ops",
              "Operations in writing to FS queue");
  plb.add_u64_counter(l_filestore_ops, "ops", "Operations written to store");
  plb.add_u64(l_filestore_op_queue_max_bytes, "op_queue_max_bytes",
              "Max data in writing to FS queue");
  plb.add_u64(l_filestore_op_queue_bytes, "op_queue_bytes",
              "Size of writing to FS queue");
  ...
  plb.add_time_avg(l_filestore_commitcycle_interval, "commitcycle_interval",
                   "Average interval between commits");
  plb.add_time_avg(l_filestore_commitcycle_latency, "commitcycle_latency",
                   "Average latency of commit");
  plb.add_u64_counter(l_filestore_journal_full, "journal_full",
                      "Journal writes while full");
  plb.add_time_avg(l_filestore_queue_transaction_latency_avg,
                   "queue_transaction_latency_avg",
                   "Store operation queue latency", NULL,
                   PerfCountersBuilder::PRIO_USEFUL);
  plb.add_time(l_filestore_sync_pause_max_lat, "sync_pause_max_latency",
               "Max latency of op_wq pause before syncfs");
...
FileStore::~FileStore() {
  ...
  if (journal) journal->logger = nullptr;
  delete logger;
  logger = nullptr;
...
int FileStore::statfs(struct rgw_filestore_statfs_t *buf0, osd_alert_list_t *alerts) {
  struct statfs buf;
  buf0->reset();
  if (alerts) {
...
void FileStore::new_journal() {
  if (journalpath.length()) {
    ...
    journal =
        new FileJournal(cct, fsid, &finisher, &sync_cond, journalpath.c_str(),
                        m_journal_dio, m_journal_aio, m_journal_force_aio);
    if (journal) journal->logger = logger;
  }
  return;
}
...
int FileStore::dump_journal(ostream &out) {
  ...
  if (!journalpath.length()) return -EINVAL;
...
FileStoreBackend *FileStoreBackend::create(unsigned long f_type,
                                           FileStore *fs) {
  switch (f_type) {
#if defined(__linux__)
...
void FileStore::create_backend(unsigned long f_type) {
  ...
  dout(0) << "backend " << backend->get_name() << " (magic 0x" << std::hex
          << f_type << std::dec << ")" << dendl;
...
int FileStore::mkfs() {
  ...
  if (basedir_fd < 0) {
    ret = -errno;
    derr << __FUNC__ << ": failed to open base dir " << basedir << ": "
  ...
  if (fsid_fd < 0) {
    ret = -errno;
    derr << __FUNC__ << ": failed to open " << fsid_fn << ": "
         << cpp_strerror(ret) << dendl;
  ...
  if (read_fsid(fsid_fd, &old_fsid) < 0 || old_fsid.is_zero()) {
    ...
    if (ret < 0) {
      ret = -errno;
      derr << __FUNC__ << ": failed to truncate fsid: " << cpp_strerror(ret)
           << dendl;
      goto close_fsid_fd;
    }
    ret = safe_write(fsid_fd, fsid_str, strlen(fsid_str));
    if (ret < 0) {
      derr << __FUNC__ << ": failed to write fsid: " << cpp_strerror(ret)
           << dendl;
      goto close_fsid_fd;
    }
    if (::fsync(fsid_fd) < 0) {
      ret = -errno;
      derr << __FUNC__
           << ": close failed: can't write fsid: " << cpp_strerror(ret)
           << dendl;
      goto close_fsid_fd;
    }
  ...
  if (ret < 0) {
    derr << __FUNC__ << ": write_version_stamp() failed: " << cpp_strerror(ret)
         << dendl;
    goto close_fsid_fd;
  }
  ...
  if (ret < 0) {
    derr << __FUNC__ << ": write_superblock() failed: " << cpp_strerror(ret)
         << dendl;
    goto close_fsid_fd;
  }
  ...
  if (ret < 0) {
    ret = -errno;
    derr << __FUNC__ << ": cannot fstatfs basedir " << cpp_strerror(ret)
         << dendl;
    goto close_fsid_fd;
  }
  ...
  if (ret < 0) {
    derr << __FUNC__ << ": failed to create current/ " << cpp_strerror(ret)
         << dendl;
    goto close_fsid_fd;
  }
  ...
  // write initial op_seq
  {
    ...
    if (initial_seq == 0) {
      ...
      if (backend->can_checkpoint()) {
        ...
        if (ret < 0 && ret != -EEXIST) {
          VOID_TEMP_FAILURE_RETRY(::close(fd));
          derr << __FUNC__ << ": failed to create snap_1: " << cpp_strerror(ret)
               << dendl;
          goto close_fsid_fd;
        }
      }
...
int FileStore::read_fsid(int fd, uuid_d *uuid) {
  ...
  if (ret > 36)
    ...
    fsid_str[ret] = 0;
  if (!uuid->parse(fsid_str)) return -EINVAL;
  return 0;
}
...
int FileStore::lock_fsid() {
  ...
  if (r < 0) {
    int err = errno;
    dout(0) << __FUNC__ << ": failed to lock " << basedir
            << "/fsid, is another ceph-osd still running? " << cpp_strerror(err)
            << dendl;
    return -err;
  }
  return 0;
...
bool FileStore::test_mount_in_use() {
  dout(5) << __FUNC__ << ": basedir " << basedir << " journal " << journalpath
          << dendl;
  char fn[PATH_MAX];
  snprintf(fn, sizeof(fn), "%s/fsid", basedir.c_str());
  ...
  if (fsid_fd < 0) return 0;  // no fsid, ok.
...
int FileStore::_sanity_check_fs() {
  ...
  if (((int)m_filestore_journal_writeahead + (int)m_filestore_journal_parallel +
       (int)m_filestore_journal_trailing) > 1) {
    dout(0) << "mount ERROR: more than one of filestore journal "
               "{writeahead,parallel,trailing} enabled"
            << dendl;
    cerr << TEXT_RED
         << " ** WARNING: more than one of 'filestore journal "
            "{writeahead,parallel,trailing}'\n"
         << "             is enabled in ceph.conf.  You must choose a single "
            "journal mode."
  ...
  if (!backend->can_checkpoint()) {
    if (!journal || !m_filestore_journal_writeahead) {
      dout(0) << "mount WARNING: no btrfs, and no journal in writeahead mode; "
                 "data may be lost"
              << dendl;
      cerr << TEXT_RED
           << " ** WARNING: no btrfs AND (no journal OR journal not in "
              "writeahead mode)\n"
           << "             For non-btrfs volumes, a writeahead journal is "
              "required to\n"
           << "             maintain on-disk consistency in the event of a "
              "crash.  Your conf\n"
  ...
  if (!journal) {
    ...
    cerr << TEXT_YELLOW
         << " ** WARNING: No osd journal is configured: write latency may be "
            "high.\n"
         << "             If you will not be using an osd journal, write "
            "latency may be\n"
         << "             relatively high.  It can be reduced somewhat by "
            "lowering\n"
         << "             filestore_max_sync_interval, but lower values mean "
            "lower write\n"
...
int FileStore::write_superblock() {
  ...
  return safe_write_file(basedir.c_str(), "superblock", bl.c_str(), bl.length(),
                         0600);
}
...
int FileStore::read_superblock() {
  ...
  int ret =
      safe_read_file(basedir.c_str(), "superblock", bp.c_str(), bp.length());
  if (ret < 0) {
    if (ret == -ENOENT) {
...
int FileStore::version_stamp_is_valid(uint32_t *version) {
  bufferptr bp(PATH_MAX);
  int ret =
      safe_read_file(basedir.c_str(), "store_version", bp.c_str(), bp.length());
  if (ret < 0) {
    return ret;
  }
  bufferlist bl;
...
int FileStore::flush_cache(ostream *os) {
  ...
  int drop_caches_fd = ::open(drop_caches_file.c_str(), O_WRONLY | O_CLOEXEC),
      ret = 0;
  char buf[2] = "3";
  ...
  if (drop_caches_fd < 0) {
    ret = -errno;
    derr << __FUNC__ << ": failed to open " << drop_caches_file << ": "
         << cpp_strerror(ret) << dendl;
    if (os) {
      *os << "FileStore flush_cache: failed to open " << drop_caches_file
          << ": " << cpp_strerror(ret);
    }
    return ret;
  }
  ...
  if (::write(drop_caches_fd, buf, len) < 0) {
    ret = -errno;
    derr << __FUNC__ << ": failed to write to " << drop_caches_file << ": "
         << cpp_strerror(ret) << dendl;
    if (os) {
      *os << "FileStore flush_cache: failed to write to " << drop_caches_file
          << ": " << cpp_strerror(ret);
    }
    goto out;
  }
...
int FileStore::write_version_stamp() {
  ...
  return safe_write_file(basedir.c_str(), "store_version", bl.c_str(),
                         bl.length(), 0600);
}
...
int FileStore::upgrade() {
  ...
  if (version < 3) {
    derr << "ObjectStore is old at version " << version
         << ".  Please upgrade to firefly v0.80.x, convert your store, and "
            "then upgrade."
         << dendl;
    return -EINVAL;
  }
...
int FileStore::read_op_seq(uint64_t *seq) {
  int op_fd =
      ::open(current_op_seq_fn.c_str(), O_CREAT | O_RDWR | O_CLOEXEC, 0644);
  if (op_fd < 0) {
    int r = -errno;
    if (r == -EIO && m_filestore_fail_eio) handle_eio();
    return r;
  ...
  if (ret < 0) {
    derr << __FUNC__ << ": error reading " << current_op_seq_fn << ": "
         << cpp_strerror(ret) << dendl;
...
int FileStore::mount() {
  ...
  if (fsid_fd < 0) {
    ret = -errno;
    derr << __FUNC__ << ": error opening '" << buf << "': " << cpp_strerror(ret)
         << dendl;
    goto done;
  }
  ...
  if (ret < 0) {
    derr << __FUNC__
         << ": error in version_stamp_is_valid: " << cpp_strerror(ret) << dendl;
    goto close_fsid_fd;
  } else if (ret == 0) {
    if (do_update || (int)version_stamp < cct->_conf->filestore_update_to) {
      derr << __FUNC__ << ": stale version stamp detected: " << version_stamp
      ...
      derr << __FUNC__ << ": stale version stamp " << version_stamp
           ...
           << " (currently " << cct->_conf->filestore_update_to << ")" << dendl;
      goto close_fsid_fd;
    }
  }
...

  {
    ...
    if (ret < 0) {
      derr << __FUNC__ << ": error in _list_snaps: " << cpp_strerror(ret)
           << dendl;
      goto close_basedir_fd;
    }
  ...
  if (m_osd_rollback_to_cluster_snap.length() &&
    ...
    derr << "rollback to cluster snapshot '" << m_osd_rollback_to_cluster_snap
         << "': not found" << dendl;
    ret = -ENOENT;
    goto close_basedir_fd;
  }
  ...
  if (backend->can_checkpoint()) {
    if (snaps.empty()) {
      dout(0) << __FUNC__
              << ": WARNING: no consistent snaps found, store may be in "
                 "inconsistent state"
              << dendl;
    } else {
      char s[NAME_MAX];
      ...
      if (m_osd_rollback_to_cluster_snap.length()) {
        derr << TEXT_RED << " ** NOTE: rolling back to cluster snapshot "
             << m_osd_rollback_to_cluster_snap << " **" << TEXT_NORMAL << dendl;
        ceph_assert(cluster_snaps.count(m_osd_rollback_to_cluster_snap));
        snprintf(s, sizeof(s), CLUSTER_SNAP_ITEM,
        ...
        if (::stat(nosnapfn, &st) == 0) {
          if (!m_osd_use_stale_snap) {
            derr << "ERROR: " << nosnapfn
                 << " exists, not rolling back to avoid losing new data"
                 << dendl;
            derr << "Force rollback to old snapshotted version with 'osd use "
                    "stale snap = true'"
                 << dendl;
            derr << "config option for --osd-use-stale-snap startup argument."
                 << dendl;
            ret = -ENOTSUP;
            goto close_basedir_fd;
          }
          derr << "WARNING: user forced start with data sequence mismatch: "
                  "current was "
               << curr_seq << ", newest snap is " << cp << dendl;
          cerr << TEXT_YELLOW
               << " ** WARNING: forcing the use of stale snapshot data **"
               << TEXT_NORMAL << std::endl;
        }

        dout(10) << __FUNC__ << ": rolling back to consistent snap " << cp
                 << dendl;
  ...
  if (current_fd < 0) {
    ret = -errno;
    derr << __FUNC__ << ": error opening: " << current_fn << ": "
         << cpp_strerror(ret) << dendl;
    goto close_basedir_fd;
  }
  ...
  // select journal mode?
  if (journal) {
    if (!m_filestore_journal_writeahead && !m_filestore_journal_parallel &&
      ...
      if (!backend->can_checkpoint()) {
        ...
        dout(0) << __FUNC__
                << ": enabling PARALLEL journal mode: fs, checkpoint is enabled"
                << dendl;
      }
    } else {
      ...
      if (m_filestore_journal_parallel)
        dout(0) << __FUNC__
                << ": PARALLEL journal mode explicitly enabled in conf"
                << dendl;
      if (m_filestore_journal_trailing)
        dout(0) << __FUNC__
                << ": TRAILING journal mode explicitly enabled in conf"
                << dendl;
    }
    if (m_filestore_journal_writeahead) journal->set_wait_on_full(true);
  } else {
  ...
  {
    vector<rgw_salcoll_t> collections;
    ret = list_collections(collections);
    if (ret < 0) {
      derr << "Error " << ret << " while listing collections" << dendl;
      goto close_current_fd;
    }
    for (vector<rgw_salcoll_t>::iterator i = collections.begin();
         i != collections.end(); ++i) {
      Index index;
      ret = get_index(*i, &index);
      if (ret < 0) {
        derr << "Unable to mount index " << *i << " with error: " << ret
             << dendl;
        goto close_current_fd;
      }
      ceph_assert(index.index);
  ...
  if (!(generic_flags & SKIP_JOURNAL_REPLAY)) {
    ...
    if (ret < 0) {
      derr << __FUNC__ << ": failed to open journal " << journalpath << ": "
           << cpp_strerror(ret) << dendl;
      if (ret == -ENOTTY) {
  ...
  op_tp.start();
  for (vector<Finisher *>::iterator it = ondisk_finishers.begin();
       it != ondisk_finishers.end(); ++it) {
    (*it)->start();
  }
  for (vector<Finisher *>::iterator it = apply_finishers.begin();
       it != apply_finishers.end(); ++it) {
    (*it)->start();
  }
...
int FileStore::umount() {
  ...
  journal_stop();
  if (!(generic_flags & SKIP_JOURNAL_REPLAY)) journal_write_close();
...
ObjectStore::CollectionHandle FileStore::open_collection(
    const rgw_salcoll_t &c) {
  std::lock_guard l{coll_lock};
...
ObjectStore::CollectionHandle FileStore::create_new_collection(
    const rgw_salcoll_t &c) {
  std::lock_guard l{coll_lock};
  auto p = coll_map.find(c);
  if (p == coll_map.end()) {
...
void FileStore::queue_op(OpSequencer *osr, Op *o) {
  ...
  dout(5) << __FUNC__ << ": " << o << " seq " << o->op << " " << *osr << " "
          << o->bytes << " bytes"
          << "   (queue has " << throttle_ops.get_current() << " ops and "
          << throttle_bytes.get_current() << " bytes)" << dendl;
  op_wq.queue(osr);
}
...
void FileStore::_do_op(OpSequencer *osr, ThreadPool::TPHandle &handle) {
  ...
  if (cct->_conf->filestore_inject_stall) {
    ...
    dout(5) << __FUNC__ << ": filestore_inject_stall " << orig << ", sleeping"
            << dendl;
    sleep(orig);
  ...
  dout(10) << __FUNC__ << ": " << o << " seq " << o->op << " r = " << r
           << ", finisher " << o->onreadable << " " << o->onreadable_sync
           << dendl;
}
...
void FileStore::_finish_op(OpSequencer *osr) {
  ...
  dout(10) << __FUNC__ << ": " << o << " seq " << o->op << " " << *osr
           << " lat " << lat << dendl;
...
struct C_JournaledAhead : public Context {
  ...
  C_JournaledAhead(FileStore *f, FileStore::OpSequencer *os, FileStore::Op *o,
                   Context *ondisk)
...
int FileStore::queue_transactions(CollectionHandle &ch,
                                  vector<Transaction> &tls, TrackedOpRef osd_op,
  ...
  if (cct->_conf->objectstore_blackhole) {
    dout(0) << __FUNC__
            << ": objectstore_blackhole = TRUE, dropping transaction" << dendl;
    delete ondisk;
    ondisk = nullptr;
    delete onreadable;
    onreadable = nullptr;
  ...
  if (osd_op && osd_op->pg_trace) {
    osd_op->store_trace.init("filestore op", &trace_endpoint,
                             &osd_op->pg_trace);
  ...
  if (journal && journal->is_writeable() && !m_filestore_journal_trailing) {
    ...
    if (m_filestore_do_dump) dump_transactions(o->tls, o->op, osr);
    ...
    if (m_filestore_journal_parallel) {
      ...
      dout(5) << __FUNC__ << ": (writeahead) " << o->op << " " << o->tls
              << dendl;
  ...
  if (!journal) {
    ...
    if (ondisk) apply_manager.add_waiter(op_num, ondisk);
    submit_manager.op_submit_finish(op_num);
    utime_t end = ceph_clock_now();
    logger->tinc(l_filestore_queue_transaction_latency_avg, end - start);
    return 0;
  ...
  if (m_filestore_do_dump) dump_transactions(tls, op, osr);
...
int FileStore::_do_transactions(vector<Transaction> &tls, uint64_t op_seq,
                                ThreadPool::TPHandle *handle,
                                const char *osr_name) {
  int trans_num = 0;

  for (vector<Transaction>::iterator p = tls.begin(); p != tls.end();
       ++p, trans_num++) {
...
void FileStore::_set_global_replay_guard(const rgw_salcoll_t &cid,
                                         const SequencerPosition &spos) {
  if (backend->can_checkpoint()) return;

  // sync all previous operations on this sequencer
  int ret = sync_filesystem(basedir_fd);
  if (ret < 0) {
    derr << __FUNC__ << ": sync_filesystem error " << cpp_strerror(ret)
         << dendl;
  ...
  bufferlist v;
  encode(spos, v);
  int r = fsetxattr(fd, GLOBAL_REPLAY_GUARD_XATTR, v.c_str(), v.length(), 0);
  if (r < 0) {
    derr << __FUNC__ << ": fsetxattr " << GLOBAL_REPLAY_GUARD_XATTR << " got "
...
int FileStore::_check_global_replay_guard(const rgw_salcoll_t &cid,
                                          const SequencerPosition &spos) {
  char fn[PATH_MAX];
  get_cdir(cid, fn, sizeof(fn));
  int fd = ::open(fn, O_RDONLY | O_CLOEXEC);
  ...
  char buf[100];
  int r = fgetxattr(fd, GLOBAL_REPLAY_GUARD_XATTR, buf, sizeof(buf));
  if (r < 0) {
...
void FileStore::_set_replay_guard(const rgw_salcoll_t &cid,
                                  const SequencerPosition &spos,
...
void FileStore::_set_replay_guard(int fd, const SequencerPosition &spos,
                                  const rgw_saloid_t *hoid, bool in_progress) {
  if (backend->can_checkpoint()) return;

  dout(10) << __FUNC__ << ": " << spos << (in_progress ? " START" : "")
           << dendl;
  ...
  r = fsetxattr(fd, REPLAY_GUARD_XATTR, v.c_str(), v.length(), 0);
  if (r < 0) {
...
void FileStore::_close_replay_guard(const rgw_salcoll_t &cid,
                                    const SequencerPosition &spos) {
  char fn[PATH_MAX];
  get_cdir(cid, fn, sizeof(fn));
  int fd = ::open(fn, O_RDONLY | O_CLOEXEC);
...
void FileStore::_close_replay_guard(int fd, const SequencerPosition &spos,
  ...
  bool in_progress = false;
  encode(in_progress, v);
  int r = fsetxattr(fd, REPLAY_GUARD_XATTR, v.c_str(), v.length(), 0);
  if (r < 0) {
    derr << "fsetxattr " << REPLAY_GUARD_XATTR << " got " << cpp_strerror(r)
         << dendl;
    ceph_abort_msg("fsetxattr failed");
...
int FileStore::_check_replay_guard(const rgw_salcoll_t &cid,
                                   const rgw_saloid_t &oid,
                                   const SequencerPosition &spos) {
  if (!replaying || backend->can_checkpoint()) return 1;
...
int FileStore::_check_replay_guard(int fd, const SequencerPosition &spos) {
  ...
  char buf[100];
  int r = fgetxattr(fd, REPLAY_GUARD_XATTR, buf, sizeof(buf));
  if (r < 0) {
    dout(20) << __FUNC__ << ": no xattr" << dendl;
    if (r == -EIO && m_filestore_fail_eio) handle_eio();
    return 1;  // no xattr
  ...
  if (opos > spos) {
    ...
    if (in_progress) {
      dout(10) << __FUNC__ << ": object has " << opos << " == current pos "
               << spos << ", in_progress=true, CONDITIONAL REPLAY" << dendl;
      return 0;
    } else {
      dout(10) << __FUNC__ << ": object has " << opos << " == current pos "
               << spos << ", in_progress=false, SKIPPING REPLAY" << dendl;
      return -1;
    }
  } else {
    dout(10) << __FUNC__ << ": object has " << opos << " < current pos " << spos
...
void FileStore::_do_transaction(Transaction &t, uint64_t op_seq, int trans_num,
                                ThreadPool::TPHandle *handle,
                                const char *osr_name) {
  ...
  while (i.have_op()) {
    ...
    switch (op->op) {
      ...
      case Transaction::OP_CREATE: {
        const rgw_salcoll_t &_cid = i.get_cid(op->cid);
        const rgw_saloid_t &oid = i.get_oid(op->oid);
        const rgw_salcoll_t &cid = _cid;
        tracepoint(objectstore, touch_enter, osr_name);
        if (_check_replay_guard(cid, oid, spos) > 0) r = _touch(cid, oid);
      ...
      case Transaction::OP_CLONE: {
        ...
        const rgw_saloid_t &oid = i.get_oid(op->oid);
        const rgw_salcoll_t &cid = _cid;
        const rgw_saloid_t &noid = i.get_oid(op->dest_oid);
      ...
      case Transaction::OP_MKCOLL: {
        const rgw_salcoll_t &cid = i.get_cid(op->cid);
      ...
      case Transaction::OP_COLL_HINT: {
        ...
        if (type == Transaction::COLL_HINT_EXPECTED_NUM_OBJECTS) {
          ...
          if (_check_replay_guard(cid, spos) > 0) {
            r = _collection_hint_expected_num_objs(cid, num_objs, spos);
          }
        } else {
          // Ignore the hint
      ...
      case Transaction::OP_RMCOLL: {
        ...
        if (_check_replay_guard(cid, spos) > 0) r = _destroy_collection(cid);
        tracepoint(objectstore, rmcoll_exit, r);
      } break;

      case Transaction::OP_COLL_ADD: {
        const rgw_salcoll_t &ocid = i.get_cid(op->cid);
        const rgw_salcoll_t &ncid = i.get_cid(op->dest_cid);
        const rgw_saloid_t &oid = i.get_oid(op->oid);

        // always followed by OP_COLL_REMOVE
        Transaction::Op *op2 = i.decode_op();
        const rgw_salcoll_t &ocid2 = i.get_cid(op2->cid);
        const rgw_saloid_t &oid2 = i.get_oid(op2->oid);
      ...
      case Transaction::OP_COLL_MOVE_RENAME: {
        const rgw_salcoll_t &oldcid = i.get_cid(op->cid);
        const rgw_saloid_t &oldoid = i.get_oid(op->oid);
        const rgw_salcoll_t &newcid = i.get_cid(op->dest_cid);
        const rgw_saloid_t &newoid = i.get_oid(op->dest_oid);
      ...
      case Transaction::OP_TRY_RENAME: {
        ...
        const rgw_saloid_t &oldoid = i.get_oid(op->oid);
        const rgw_saloid_t &newoid = i.get_oid(op->dest_oid);
        const rgw_salcoll_t &oldcid = _cid;
        const rgw_salcoll_t &newcid = _cid;
        tracepoint(objectstore, rgw_salcoll_try_rename_enter);
        r = _collection_move_rename(oldcid, oldoid, newcid, newoid, spos, true);
        tracepoint(objectstore, rgw_salcoll_try_rename_exit, r);
      } break;
    ...
    if (r < 0) {
      ...
      if (!ok) {
        ...
        if (r == -ENOENT && (op->op == Transaction::OP_CLONERANGE ||
          ...
          msg =
              "EPERM suggests file(s) in osd data dir not owned by ceph user, "
              "or leveldb corruption";
        }

        derr << " error " << cpp_strerror(r) << " not handled on operation "
...
bool FileStore::exists(CollectionHandle &ch, const rgw_saloid_t &oid) {
...
int FileStore::stat(CollectionHandle &ch, const rgw_saloid_t &oid,
                    struct stat *st, bool allow_eio) {
  tracepoint(objectstore, stat_enter, ch->cid.c_str());
  auto osr = static_cast<OpSequencer *>(ch.get());
  osr->wait_for_apply(oid);
  const rgw_salcoll_t &cid = ch->cid;
  int r = lfn_stat(cid, oid, st);
  ceph_assert(allow_eio || !m_filestore_fail_eio || r != -EIO);
  if (r < 0) {
    dout(10) << __FUNC__ << ": " << ch->cid << "/" << oid << " = " << r
             << dendl;
  } else {
    dout(10) << __FUNC__ << ": " << ch->cid << "/" << oid << " = " << r
             << " (size " << st->st_size << ")" << dendl;
  }
  if (cct->_conf->filestore_debug_inject_read_err && debug_mdata_eio(oid)) {
    return -EIO;
  } else {
...
int FileStore::read(CollectionHandle &ch, const rgw_saloid_t &oid,
  ...
  if (r < 0) {
    dout(10) << __FUNC__ << ": (" << cid << "/" << oid
             << ") open error: " << cpp_strerror(r) << dendl;
    return r;
  }
  ...
  if (got < 0) {
    dout(10) << __FUNC__ << ": (" << cid << "/" << oid
             << ") pread error: " << cpp_strerror(got) << dendl;
    lfn_close(fd);
    return got;
  }
  ...
  if (op_flags &
      (CEPH_OSD_OP_FLAG_FADVISE_RANDOM | CEPH_OSD_OP_FLAG_FADVISE_SEQUENTIAL))
  ...
  dout(10) << __FUNC__ << ": " << cid << "/" << oid << " " << offset << "~"
           << got << "/" << len << dendl;
  if (cct->_conf->filestore_debug_inject_read_err && debug_data_eio(oid)) {
    return -EIO;
...
more:
  ...
  while (i < fiemap->fm_mapped_extents) {
    ...
    dout(10) << __FUNC__ << ": fm_mapped_extents=" << fiemap->fm_mapped_extents
             << " fe_logical=" << extent->fe_logical
             << " fe_length=" << extent->fe_length << dendl;
...
int FileStore::fiemap(CollectionHandle &ch, const rgw_saloid_t &oid,
                      uint64_t offset, size_t len, bufferlist &bl) {
...
int FileStore::fiemap(CollectionHandle &ch, const rgw_saloid_t &oid,
  ...
  int r = lfn_open(cid, oid, false, &fd);
  if (r < 0) {
    dout(10) << "read couldn't open " << cid << "/" << oid << ": "
             << cpp_strerror(r) << dendl;
    goto done;
  }

  if (backend->has_seek_data_hole()) {
    dout(15) << "seek_data/seek_hole " << cid << "/" << oid << " " << offset
             << "~" << len << dendl;
    r = _do_seek_hole_data(**fd, offset, len, &destmap);
  } else if (backend->has_fiemap()) {
    dout(15) << "fiemap ioctl" << cid << "/" << oid << " " << offset << "~"
             << len << dendl;
  ...
  dout(10) << __FUNC__ << ": " << cid << "/" << oid << " " << offset << "~"
           << len << " = " << r << " num_extents=" << destmap.size() << " "
           << destmap << dendl;
  if (r == -EIO && m_filestore_fail_eio) handle_eio();
...
int FileStore::_remove(const rgw_salcoll_t &cid, const rgw_saloid_t &oid,
                       const SequencerPosition &spos) {
...
int FileStore::_truncate(const rgw_salcoll_t &cid, const rgw_saloid_t &oid,
                         uint64_t size) {
...
int FileStore::_touch(const rgw_salcoll_t &cid, const rgw_saloid_t &oid) {
  dout(15) << __FUNC__ << ": " << cid << "/" << oid << dendl;
...
int FileStore::_write(const rgw_salcoll_t &cid, const rgw_saloid_t &oid,
  ...
  if (r < 0) {
    dout(0) << __FUNC__ << ": couldn't open " << cid << "/" << oid << ": "
            << cpp_strerror(r) << dendl;
    goto out;
  }
...
int FileStore::_zero(const rgw_salcoll_t &cid, const rgw_saloid_t &oid,
                     uint64_t offset, size_t len) {
  dout(15) << __FUNC__ << ": " << cid << "/" << oid << " " << offset << "~"
           << len << dendl;
  int ret = 0;
  ...
  if (cct->_conf->filestore_punch_hole) {
    ...
    // first try fallocate
    ret = fallocate(**fd, FALLOC_FL_KEEP_SIZE | FALLOC_FL_PUNCH_HOLE, offset,
                    len);
    if (ret < 0) {
      ret = -errno;
    } else {
    ...
    if (ret == 0) goto out;            // yay!
    if (ret != -EOPNOTSUPP) goto out;  // some other error
#endif
#endif
#endif
  }
...
int FileStore::_clone(const rgw_salcoll_t &cid, const rgw_saloid_t &oldoid,
                      const rgw_saloid_t &newoid,
                      const SequencerPosition &spos) {
  dout(15) << __FUNC__ << ": " << cid << "/" << oldoid << " -> " << cid << "/"
           << newoid << dendl;

  if (_check_replay_guard(cid, newoid, spos) < 0) return 0;
...
int FileStore::_do_clone_range(int from, int to, uint64_t srcoff, uint64_t len,
                               uint64_t dstoff) {
  dout(20) << __FUNC__ << ": copy " << srcoff << "~" << len << " to " << dstoff
           << dendl;
  return backend->clone_range(from, to, srcoff, len, dstoff);
}

int FileStore::_do_sparse_copy_range(int from, int to, uint64_t srcoff,
                                     uint64_t len, uint64_t dstoff) {
  dout(20) << __FUNC__ << ": " << srcoff << "~" << len << " to " << dstoff
           << dendl;
  int r = 0;
  map<uint64_t, uint64_t> exomap;
  ...
  if (backend->has_seek_data_hole()) {
    dout(15) << "seek_data/seek_hole " << from << " " << srcoff << "~" << len
             << dendl;
  ...
  for (map<uint64_t, uint64_t>::iterator miter = exomap.begin();
       miter != exomap.end(); ++miter) {
    uint64_t it_off = miter->first - srcoff + dstoff;
    r = _do_copy_range(from, to, miter->first, miter->second, it_off, true);
    if (r < 0) {
      derr << __FUNC__ << ": copy error at " << miter->first << "~"
           << miter->second << " to " << it_off << ", " << cpp_strerror(r)
           << dendl;
      break;
    }
  ...
  if (r >= 0) {
    ...
    if (r < 0) {
      r = -errno;
      derr << __FUNC__ << ": fstat error at " << to << " " << cpp_strerror(r)
           << dendl;
      goto out;
    }
    if (st.st_size < (int)(dstoff + len)) {
      ...
      if (r < 0) {
        r = -errno;
        derr << __FUNC__ << ": ftruncate error at " << dstoff + len << " "
             << cpp_strerror(r) << dendl;
        goto out;
      }
    }
...
int FileStore::_do_copy_range(int from, int to, uint64_t srcoff, uint64_t len,
                              uint64_t dstoff, bool skip_sloppycrc) {
  dout(20) << __FUNC__ << ": " << srcoff << "~" << len << " to " << dstoff
           << dendl;
  int r = 0;
  loff_t pos = srcoff;
  ...
  if (backend->has_splice()) {
    ...
    while (pos < end) {
      ...
      dout(10) << "  safe_splice read from " << pos << "~" << l << " got " << r
               << dendl;
      if (r < 0) {
      ...
      if (r == 0) {
        ...
        r = -ERANGE;
        derr << __FUNC__ << ": got short read result at " << pos << " of fd "
             << from << " len " << len << dendl;
        break;
      }

      r = safe_splice(pipefd[0], nullptr, to, &dstpos, r, 0);
      dout(10) << " safe_splice write to " << to << " len " << r << " got " << r
               << dendl;
      if (r < 0) {
        derr << __FUNC__ << ": write error at " << pos << "~" << r << ", "
             << cpp_strerror(r) << dendl;
        break;
      }
    }
...
#endif
  {
    ...
    while (pos < end) {
      ...
      while (op < r) {
        ...
        dout(25) << " write to " << to << " len " << (r - op) << " got " << r2
                 << dendl;
        if (r2 < 0) {
          r = r2;
          derr << __FUNC__ << ": write error at " << pos << "~" << r - op
               << ", " << cpp_strerror(r) << dendl;
  ...
  if (r < 0 && replaying) {
    ...
    derr << __FUNC__ << ": short source tolerated because we are replaying"
         << dendl;
    r = len;
  }
...
int FileStore::_clone_range(const rgw_salcoll_t &oldcid,
                            const rgw_saloid_t &oldoid,
                            const rgw_salcoll_t &newcid,
                            const rgw_saloid_t &newoid, uint64_t srcoff,
                            uint64_t len, uint64_t dstoff,
                            const SequencerPosition &spos) {
  dout(15) << __FUNC__ << ": " << oldcid << "/" << oldoid << " -> " << newcid
           << "/" << newoid << " " << srcoff << "~" << len << " to " << dstoff
           << dendl;

  if (_check_replay_guard(newcid, newoid, spos) < 0) return 0;
...
out:
  lfn_close(o);
out2:
  dout(10) << __FUNC__ << ": " << oldcid << "/" << oldoid << " -> " << newcid
           << "/" << newoid << " " << srcoff << "~" << len << " to " << dstoff
           << " = " << r << dendl;
  return r;
}
...
void FileStore::sync_entry() {
  ...
  while (!stop) {
    ...
    if (!force_sync) {
      dout(20) << __FUNC__ << ":  waiting for max_interval " << max_interval
               << dendl;
  ...
  again:
    ...
    if (apply_manager.commit_start()) {
      ...
      dout(10) << __FUNC__ << ": commit took " << lat << ", interval was "
               << dur << dendl;
    ...
    if (!stop && journal && journal->should_commit_now()) {
      dout(10) << __FUNC__
               << ": journal says we should commit again (probably is/was full)"
               << dendl;
      goto again;
    }
  }
...
// debug EIO injection
void FileStore::inject_data_error(const rgw_saloid_t &oid) {
  std::lock_guard l{read_error_lock};
  dout(10) << __FUNC__ << ": init error on " << oid << dendl;
  data_error_set.insert(oid);
}
void FileStore::inject_mdata_error(const rgw_saloid_t &oid) {
  std::lock_guard l{read_error_lock};
  dout(10) << __FUNC__ << ": init error on " << oid << dendl;
  mdata_error_set.insert(oid);
}

void FileStore::debug_obj_on_delete(const rgw_saloid_t &oid) {
  std::lock_guard l{read_error_lock};
  dout(10) << __FUNC__ << ": clear error on " << oid << dendl;
  data_error_set.erase(oid);
  mdata_error_set.erase(oid);
}
bool FileStore::debug_data_eio(const rgw_saloid_t &oid) {
  std::lock_guard l{read_error_lock};
...
bool FileStore::debug_mdata_eio(const rgw_saloid_t &oid) {
  std::lock_guard l{read_error_lock};
...
int FileStore::_collection_remove_recursive(const rgw_salcoll_t &cid,
                                            const SequencerPosition &spos) {
  struct stat st;
  ...
  vector<rgw_saloid_t> objects;
  rgw_saloid_t max;
  while (!max.is_max()) {
    r = collection_list(cid, max, rgw_saloid_t::get_max(), 300, &objects, &max);
    if (r < 0) return r;
    for (vector<rgw_saloid_t>::iterator i = objects.begin(); i != objects.end();
         ++i) {
...
int FileStore::list_collections(vector<rgw_salcoll_t> &ls) {
...
int FileStore::collection_stat(const rgw_salcoll_t &c, struct stat *st) {
...
bool FileStore::collection_exists(const rgw_salcoll_t &c) {
...
int FileStore::collection_empty(const rgw_salcoll_t &cid, bool *empty) {
  ...
  r = index->collection_list_partial(rgw_saloid_t(), rgw_saloid_t::get_max(), 1,
                                     &ls, nullptr);
  if (r < 0) {
    derr << __FUNC__
         << ": collection_list_partial returned: " << cpp_strerror(r) << dendl;
    if (r == -EIO && m_filestore_fail_eio) handle_eio();
    return r;
  }
  *empty = ls.empty();
...
int FileStore::_collection_set_bits(const rgw_salcoll_t &c, int bits) {
  char fn[PATH_MAX];
  get_cdir(c, fn, sizeof(fn));
  ...
  r = fsetxattr(fd, n, (char *)&v, sizeof(v), 0);
  VOID_TEMP_FAILURE_RETRY(::close(fd));
out:
...
int FileStore::collection_bits(CollectionHandle &ch) {
  ...
  get_attrname("bits", n, PATH_MAX);
  r = fgetxattr(fd, n, (char *)&bits, sizeof(bits));
  VOID_TEMP_FAILURE_RETRY(::close(fd));
  if (r < 0) {
    bits = r;
    goto out;
...
int FileStore::collection_list(const rgw_salcoll_t &c,
                               const rgw_saloid_t &orig_start,
                               const rgw_saloid_t &end, int max,
                               vector<rgw_saloid_t> *ls, rgw_saloid_t *next) {
  rgw_saloid_t start = orig_start;
  if (start.is_max()) return 0;

  rgw_saloid_t temp_next;
  if (!next) next = &temp_next;
  rgw_saloid_t sep;
...
int FileStore::_collection_hint_expected_num_objs(
    const rgw_salcoll_t &c, uint64_t expected_num_objs,
    const SequencerPosition &spos) {
  dout(15) << __FUNC__ << ": collection: " << c
           << " expected number of objects: " << expected_num_objs << dendl;
  ...
  if (!empty && !replaying) {
    dout(0)
        << "Failed to give an expected number of objects hint to collection : "
  ...
  ret = index->pre_hash_collection(expected_num_objs);
...
int FileStore::_create_collection(const rgw_salcoll_t &c, int bits,
                                  const SequencerPosition &spos) {
  char fn[PATH_MAX];
  get_cdir(c, fn, sizeof(fn));
  dout(15) << __FUNC__ << ": " << fn << dendl;
...
int FileStore::_destroy_collection(const rgw_salcoll_t &c) {
  int r = 0;
  char fn[PATH_MAX];
  get_cdir(c, fn, sizeof(fn));
  dout(15) << __FUNC__ << ": " << fn << dendl;
...
int FileStore::_collection_move_rename(const rgw_salcoll_t &oldcid,
                                       const rgw_saloid_t &oldoid,
                                       rgw_salcoll_t c, const rgw_saloid_t &o,
                                       const SequencerPosition &spos,
                                       bool allow_enoent) {
  dout(15) << __FUNC__ << ": " << c << "/" << o << " from " << oldcid << "/"
           << oldoid << dendl;
  ...
  if (replaying) {
     ...
     */
    if (!collection_exists(c)) goto out_rm_src;
  }

  dstcmp = _check_replay_guard(c, o, spos);
  if (dstcmp < 0) goto out_rm_src;
...

  {
    ...
    if (r < 0) {
      ...
      if (replaying) {
        dout(10) << __FUNC__ << ": " << c << "/" << o << " from " << oldcid
                 << "/" << oldoid << " (dne, continue replay) " << dendl;
      } else if (allow_enoent) {
        dout(10) << __FUNC__ << ": " << c << "/" << o << " from " << oldcid
                 << "/" << oldoid << " (dne, ignoring enoent)" << dendl;
      } else {
      ...
      if (allow_enoent &&
          dstcmp > 0) {  // if dstcmp == 0, try_rename was started.
        return 0;
      }
    ...
    if (r == 0) r = lfn_unlink(oldcid, oldoid, spos);

    if (r == 0) r = lfn_open(c, o, 0, &fd);
...
out_rm_src:
  ...
  if (_check_replay_guard(oldcid, oldoid, spos) > 0) {
    r = lfn_unlink(oldcid, oldoid, spos);
  }
...
int FileStore::_merge_collection(const rgw_salcoll_t &cid, uint32_t bits,
                                 rgw_salcoll_t dest,
                                 const SequencerPosition &spos) {
  ...
  // set bits
  if (_check_replay_guard(cid, spos) > 0) _collection_set_bits(dest, bits);
  ...
  if (!r && cct->_conf->filestore_debug_verify_split) {
    ...
    rgw_saloid_t next;
    while (1) {
      collection_list(dest, next, rgw_saloid_t::get_max(), get_ideal_list_max(),
                      &objects, &next);
...
int FileStore::_split_collection(const rgw_salcoll_t &cid, uint32_t bits,
                                 uint32_t rem, rgw_salcoll_t dest,
                                 const SequencerPosition &spos) {
  int r;
  {
  ...
  if (!r && cct->_conf->filestore_debug_verify_split) {
    ...
    rgw_saloid_t next;
    while (1) {
      collection_list(cid, next, rgw_saloid_t::get_max(), get_ideal_list_max(),
                      &objects, &next);
      if (objects.empty()) break;
      for (vector<rgw_saloid_t>::iterator i = objects.begin();
           i != objects.end(); ++i) {
        dout(20) << __FUNC__ << ": " << *i << " still in source " << cid
                 << dendl;
...
int FileStore::_set_alloc_hint(const rgw_salcoll_t &cid,
                               const rgw_saloid_t &oid,
  ...
  if (expected_object_size == 0 || expected_write_size == 0) goto out;
...

  {
    ...
    uint64_t hint = std::min<uint64_t>(expected_write_size,
                                       m_filestore_max_alloc_hint_size);
...
const char **FileStore::get_tracked_conf_keys() const {
  static const char *KEYS[] = {"filestore_max_inline_xattr_size",
...
void FileStore::handle_conf_change(const ConfigProxy &conf,
  ...
  if (changed.count("filestore_op_thread_suicide_timeout")) {
    op_wq.set_suicide_timeout(
        g_conf().get_val<int64_t>("filestore_op_thread_suicide_timeout"));
  }
}
...
int FileStore::set_throttle_params() {
  ...
  bool valid = throttle_bytes.set_params(
      ...
      cct->_conf->filestore_queue_max_bytes, &ss);
  ...
  valid &= throttle_ops.set_params(
      ...
      cct->_conf->filestore_queue_max_ops, &ss);
...
void FileStore::dump_transactions(vector<ObjectStore::Transaction> &ls,
                                  uint64_t seq, OpSequencer *osr) {
  m_filestore_dump_fmt.open_array_section("transactions");
  unsigned trans_num = 0;
  for (vector<ObjectStore::Transaction>::iterator i = ls.begin(); i != ls.end();
       ++i, ++trans_num) {
...
void FileStore::set_xattr_limits_via_conf() {
  ...
  switch (m_fs_type) {
    ...
    case BTRFS_SUPER_MAGIC:
      ...
      fs_xattr_max_value_size =
          cct->_conf->filestore_max_xattr_value_size_btrfs;
      break;
#endif
    default:
      ...
      fs_xattr_max_value_size =
          cct->_conf->filestore_max_xattr_value_size_other;
      break;
  }

  // Use override value if set
  if (cct->_conf->filestore_max_inline_xattr_size)
    m_filestore_max_inline_xattr_size =
        cct->_conf->filestore_max_inline_xattr_size;
  else
  ...
  if (cct->_conf->filestore_max_xattr_value_size)
    m_filestore_max_xattr_value_size =
        cct->_conf->filestore_max_xattr_value_size;
  else
  ...
  if (m_filestore_max_xattr_value_size < cct->_conf->osd_max_object_name_len) {
    derr << "WARNING: max attr value size (" << m_filestore_max_xattr_value_size
...
uint64_t FileStore::estimate_objects_overhead(uint64_t num_objects) {
  uint64_t res =
      num_objects * blk_size /
      2;  //assumes that each object uses ( in average ) additional 1/2 block due to FS allocation granularity.
  return res;
}

int FileStore::apply_layout_settings(const rgw_salcoll_t &cid,
                                     int target_level) {
  dout(20) << __FUNC__ << ": " << cid << " target level: " << target_level
           << dendl;
  Index index;
  int r = get_index(cid, &index);
  if (r < 0) {
...
void FSSuperblock::generate_test_instances(list<FSSuperblock *> &o) {
  ...
  z.compat_features =
      CompatSet(feature_compat, feature_ro_compat, feature_incompat);
  o.push_back(new FSSuperblock(z));
  o.push_back(new FSSuperblock(z));
}
...
void FileStore::OpSequencer::_register_apply(Op *o) {
  ...
  for (auto &t : o->tls) {
    for (auto &i : t.get_object_index()) {
      uint32_t key = i.first.get_hash();
      applying.emplace(make_pair(key, &i.first));
      dout(20) << __func__ << " " << o << " " << i.first << " (" << &i.first
               << ")" << dendl;
    }
  }
}
...
void FileStore::OpSequencer::_unregister_apply(Op *o) {
  ...
  for (auto &t : o->tls) {
    for (auto &i : t.get_object_index()) {
      ...
      bool removed = false;
      while (p != applying.end() && p->first == key) {
        if (p->second == &i.first) {
          dout(20) << __func__ << " " << o << " " << i.first << " (" << &i.first
                   << ")" << dendl;
          applying.erase(p);
...
void FileStore::OpSequencer::wait_for_apply(const rgw_saloid_t &oid) {
  std::unique_lock l{qlock};
  uint32_t key = oid.get_hash();
retry:
  while (true) {
