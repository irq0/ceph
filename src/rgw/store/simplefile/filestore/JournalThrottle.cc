...
bool JournalThrottle::set_params(double _low_threshhold,
                                 double _high_threshhold,
                                 double _expected_throughput,
                                 double _high_multiple, double _max_multiple,
                                 uint64_t _throttle_max,
                                 std::ostream *errstream) {
  return throttle.set_params(_low_threshhold, _high_threshhold,
                             _expected_throughput, _high_multiple,
                             _max_multiple, _throttle_max, errstream);
}
...
std::pair<uint64_t, uint64_t> JournalThrottle::flush(uint64_t mono_id) {
  ...
  {
    locker l(lock);
    while (!journaled_ops.empty() && journaled_ops.front().first <= mono_id) {
