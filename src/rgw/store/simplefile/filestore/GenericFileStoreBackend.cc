...
#undef dout_prefix
#define dout_prefix \
  *_dout << "genericfilestorebackend(" << get_basedir_path() << ") "
...
GenericFileStoreBackend::GenericFileStoreBackend(FileStore *fs)
  ...
  // rotational?
  {
    ...
    dout(20) << __func__ << " basedir " << fn << " rotational "
             << (int)m_rotational << dendl;
    ::close(fd);
  }
  // journal rotational?
  {
...
int GenericFileStoreBackend::detect_features() {
  ...
  if (fd < 0) {
    fd = -errno;
    derr << "detect_features: unable to create " << fn << ": "
  ...
  for (int i = 0; v[i]; i++) {
    ...
    if (r < 0) {
      r = -errno;
      derr << "detect_features: failed to lseek " << fn << ": "
    ...
    if (r < 0) {
      derr << "detect_features: failed to write to " << fn << ": "
           << cpp_strerror(r) << dendl;
      VOID_TEMP_FAILURE_RETRY(::close(fd));
      return r;
    }
  ...
  if (!m_filestore_fiemap) {
    dout(0) << "detect_features: FIEMAP ioctl is disabled via 'filestore "
    ...
    if (r < 0) {
      ...
      if (fiemap->fm_mapped_extents == 0) {
        dout(0) << "detect_features: FIEMAP ioctl is supported, but buggy -- "
                   "upgrade your kernel"
                << dendl;
        ioctl_fiemap = false;
      } else {
  ...
  if (!m_filestore_seek_data_hole) {
    dout(0) << "detect_features: SEEK_DATA/SEEK_HOLE is disabled via "
               "'filestore seek data hole' config option"
            << dendl;
    seek_data_hole = false;
  } else {
    ...
    if (hole_pos < 0) {
      if (errno == EINVAL) {
        dout(0) << "detect_features: lseek SEEK_DATA/SEEK_HOLE is NOT supported"
                << dendl;
        seek_data_hole = false;
      } else {
        derr << "detect_features: failed to lseek " << fn << ": "
      ...
      dout(0) << "detect_features: lseek SEEK_DATA/SEEK_HOLE is supported"
              << dendl;
      seek_data_hole = true;
    }
#endif
  ...
  if (!m_filestore_splice) {
    ...
    if (pipe_cloexec(pipefd, 0) < 0) {
      int e = errno;
      dout(0) << "detect_features: splice pipe met error " << cpp_strerror(e)
              << dendl;
    } else {
      lseek(fd, 0, SEEK_SET);
  ...
  if (::syncfs(get_basedir_fd()) == 0) {
    dout(0) << "detect_features: syncfs(2) syscall fully supported (by glibc "
               "and kernel)"
            << dendl;
    have_syncfs = true;
  } else {
    dout(0) << "detect_features: syncfs(2) syscall supported by glibc BUT NOT "
               "the kernel"
            << dendl;
  }
#elif defined(SYS_syncfs)
  if (syscall(SYS_syncfs, get_basedir_fd()) == 0) {
    dout(0) << "detect_features: syscall(SYS_syncfs, fd) fully supported"
            << dendl;
    have_syncfs = true;
  } else {
    dout(0) << "detect_features: syscall(SYS_syncfs, fd) supported by libc BUT "
               "NOT the kernel"
            << dendl;
  }
#elif defined(__NR_syncfs)
  if (syscall(__NR_syncfs, get_basedir_fd()) == 0) {
    dout(0) << "detect_features: syscall(__NR_syncfs, fd) fully supported"
            << dendl;
    have_syncfs = true;
  } else {
    dout(0) << "detect_features: syscall(__NR_syncfs, fd) supported by libc "
               "BUT NOT the kernel"
            << dendl;
  }
#endif
  if (!have_syncfs) {
    ...
    if (m_filestore_fsync_flushes_journal_data) {
      dout(0) << "detect_features: no syncfs(2), but 'filestore fsync flushes "
                 "journal data = true', so fsync will suffice."
              << dendl;
    } else {
      dout(0) << "detect_features: no syncfs(2), must use sync(2)." << dendl;
      dout(0) << "detect_features: WARNING: multiple ceph-osd daemons on the "
                 "same host will be slow"
              << dendl;
    }
  }
...
int GenericFileStoreBackend::create_current() {
  ...
  if (ret == 0) {
    ...
    if (!S_ISDIR(st.st_mode)) {
      dout(0) << "_create_current: current/ exists but is not a directory"
              << dendl;
      ret = -EINVAL;
    }
  } else {
    ...
    if (ret < 0) {
      ret = -errno;
      dout(0) << "_create_current: mkdir " << get_current_path()
...
int GenericFileStoreBackend::do_fiemap(int fd, off_t start, size_t len,
  ...
  _realloc_fiemap =
      (struct fiemap *)realloc(fiemap, sizeof(struct fiemap) + size);
  if (!_realloc_fiemap) {
    ret = -ENOMEM;
    goto done_err;
  } else {
...
int GenericFileStoreBackend::_crc_load_or_init(int fd, SloppyCRCMap *cm) {
  ...
  bufferptr bp;
  int r = 0;
  int l = fgetxattr(fd, SLOPPY_CRC_XATTR, buf, sizeof(buf));
  if (l == -ENODATA) {
    return 0;
  }
  if (l >= 0) {
    ...
    l = fgetxattr(fd, SLOPPY_CRC_XATTR, 0, 0);
    if (l > 0) {
      bp = ceph::buffer::create(l);
      l = fgetxattr(fd, SLOPPY_CRC_XATTR, bp.c_str(), l);
    }
  }
  bufferlist bl;
  ...
  if (r < 0) derr << __func__ << " got " << cpp_strerror(r) << dendl;
  return r;
}
...
int GenericFileStoreBackend::_crc_save(int fd, SloppyCRCMap *cm) {
  bufferlist bl;
  encode(*cm, bl);
  int r = fsetxattr(fd, SLOPPY_CRC_XATTR, bl.c_str(), bl.length(), 0);
  if (r < 0) derr << __func__ << " got " << cpp_strerror(r) << dendl;
  return r;
}

int GenericFileStoreBackend::_crc_update_write(int fd, loff_t off, size_t len,
                                               const bufferlist &bl) {
...
int GenericFileStoreBackend::_crc_verify_read(int fd, loff_t off, size_t len,
                                              const bufferlist &bl,
                                              ostream *out) {
  SloppyCRCMap scm(get_crc_block_size());
  int r = _crc_load_or_init(fd, &scm);
