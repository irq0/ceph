...
class FDCache : public md_config_obs_t {
 ...
 private:
  ...
  SharedLRU<rgw_saloid_t, FD> *registry;

 public:
  explicit FDCache(CephContext *cct)
      : cct(cct),
        registry_shards(
            std::max<int64_t>(cct->_conf->filestore_fd_cache_shards, 1)) {
    ceph_assert(cct);
    cct->_conf.add_observer(this);
    registry = new SharedLRU<rgw_saloid_t, FD>[registry_shards];
    for (int i = 0; i < registry_shards; ++i) {
      registry[i].set_cct(cct);
      registry[i].set_size(std::max<int64_t>(
          (cct->_conf->filestore_fd_cache_size / registry_shards), 1));
    }
  }
  ~FDCache() override {
  ...
  FDRef lookup(const rgw_saloid_t &hoid) {
    int registry_id = hoid.get_hash() % registry_shards;
    return registry[registry_id].lookup(hoid);
  }

  FDRef add(const rgw_saloid_t &hoid, int fd, bool *existed) {
    int registry_id = hoid.get_hash() % registry_shards;
    return registry[registry_id].add(hoid, new FD(fd), existed);
  }

  /// clear cached fd for hoid, subsequent lookups will get an empty FD
  void clear(const rgw_saloid_t &hoid) {
    int registry_id = hoid.get_hash() % registry_shards;
  ...
  const char **get_tracked_conf_keys() const override {
    static const char *KEYS[] = {"filestore_fd_cache_size", NULL};
    return KEYS;
  }
  void handle_conf_change(const ConfigProxy &conf,
    ...
    if (changed.count("filestore_fd_cache_size")) {
      for (int i = 0; i < registry_shards; ++i)
        registry[i].set_size(std::max<int64_t>(
            (conf->filestore_fd_cache_size / registry_shards), 1));
    }
  }
};
