...
class Journal {
 ...
 public:
  Journal(CephContext *cct, uuid_d f, Finisher *fin,
          ceph::condition_variable *c = 0)
      : fsid(f),
        finisher(fin),
        cct(cct),
        logger(NULL),
  ...
  virtual int dump(std::ostream &out) {
    return -EOPNOTSUPP;
  }
  ...
  virtual void submit_entry(uint64_t seq, ceph::buffer::list &e,
                            uint32_t orig_len, Context *oncommit,
  ...
  virtual int prepare_entry(std::vector<ObjectStore::Transaction> &tls,
                            ceph::buffer::list *tbl) = 0;

  virtual off64_t get_journal_size_estimate() {
    return 0;
  }
