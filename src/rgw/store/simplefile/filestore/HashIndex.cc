...
string reverse_hexdigit_bits_string(string s) {
  for (unsigned i = 0; i < s.size(); ++i) s[i] = reverse_hexdigit_bits(s[i]);
  return s;
}
...
int HashIndex::reset_attr(const vector<string> &path) {
  ...
  if (r < 0) return r;
  if (!exists) return 0;
  map<string, rgw_saloid_t> objects;
...
int HashIndex::col_split_level(HashIndex &from, HashIndex &to,
  ...
  for (vector<string>::iterator i = subdirs.begin(); i != subdirs.end(); ++i) {
    ...
    if (bits < inbits) {
      if (rgw_saloid_t::match_hash(hash, bits, match)) {
        r = col_split_level(from, to, sub_path, inbits, match, mkdirred);
        if (r < 0) return r;
        if (*mkdirred > path.size()) *mkdirred = path.size();
      }  // else, skip, doesn't need to be moved or recursed into
    } else {
      if (rgw_saloid_t::match_hash(hash, inbits, match)) {
        to_move.insert(*i);
      }
    }  // else, skip, doesn't need to be moved or recursed into
  }
  ...
  map<string, rgw_saloid_t> objs_to_move;
  for (map<string, rgw_saloid_t>::iterator i = objects.begin();
       i != objects.end(); ++i) {
  ...
  if (objs_to_move.empty() && to_move.empty()) return 0;
  ...
  while (*mkdirred < path.size()) {
    ...
    if (*mkdirred < path.size() - 1) info.subdirs = 1;
  ...
  for (map<string, rgw_saloid_t>::iterator i = objs_to_move.begin();
...
int HashIndex::_created(const vector<string> &path, const rgw_saloid_t &oid,
...
int HashIndex::_remove(const vector<string> &path, const rgw_saloid_t &oid,
                       const string &mangled_name) {
  int r;
...
int HashIndex::_lookup(const rgw_saloid_t &oid, vector<string> *path,
  ...
  while (1) {
    ...
    if (next == path_comp.end()) break;
    path->push_back(*(next++));
  }
  return get_mangled_name(*path, oid, mangled_name, hardlink);
}

int HashIndex::_collection_list_partial(const rgw_saloid_t &start,
...
int HashIndex::_pre_hash_collection(uint64_t expected_num_objs) {
  int ret;
  vector<string> path;
...
int HashIndex::pre_split_folder(uint64_t expected_num_objs) {
  ...
  // No need to split
  if (leavies == 0 || expected_num_objs == objs_per_folder) return 0;

  int ret;
  std::vector<std::string> paths;

  int split_bits = 32;  // XXX makes sense?

  const uint32_t subs = (1 << split_bits);
  // Calculate how many levels we create starting from here
  int level = 0;
  int level_limit = MAX_HASH_LEVEL;
...
int HashIndex::_recursive_remove(const vector<string> &path, bool top) {
  ...
  r = list_objects(path, 0, 0, &objects);
  if (r < 0) return r;
  if (!objects.empty()) return -ENOTEMPTY;
...
int HashIndex::complete_split(const vector<string> &path, subdir_info_s info) {
  ...
  map<string, map<string, rgw_saloid_t> > mapped;
  map<string, rgw_saloid_t> moved;
  int num_moved = 0;
  for (map<string, rgw_saloid_t>::iterator i = objects.begin();
       i != objects.end(); ++i) {
  ...
  for (map<string, map<string, rgw_saloid_t> >::iterator i = mapped.begin();
       i != mapped.end();) {
    dst[level] = i->first;
    ...
    if (subdirs.count(i->first) && !get_info(dst, &temp)) {
      for (map<string, rgw_saloid_t>::iterator j = i->second.begin();
           j != i->second.end(); ++j) {
...
void HashIndex::get_path_components(const rgw_saloid_t &oid,
                                    vector<string> *path) {
  char buf[MAX_HASH_LEVEL + 1];
  snprintf(buf, sizeof(buf), "%.*X", MAX_HASH_LEVEL,
           (uint32_t)oid.get_nibblewise_key());

  // Path components are the hex characters of oid.hash, least
  // significant first
...
string HashIndex::get_path_str(const rgw_saloid_t &oid) {
  ceph_assert(!oid.is_max());
  return get_hash_str(oid.get_hash());
}
...
int HashIndex::get_path_contents_by_hash_bitwise(
    const vector<string> &path, const rgw_saloid_t *next_object,
    set<string, CmpHexdigitStringBitwise> *hash_prefixes,
    set<pair<string, rgw_saloid_t>, CmpPairBitwise> *objects) {
  map<string, rgw_saloid_t> rev_objects;
  int r;
  r = list_objects(path, 0, 0, &rev_objects);
  if (r < 0) return r;
  // bitwise sort
  for (map<string, rgw_saloid_t>::iterator i = rev_objects.begin();
       i != rev_objects.end(); ++i) {
    if (next_object && i->second < *next_object) continue;
    string hash_prefix = get_path_str(i->second);
    hash_prefixes->insert(hash_prefix);
    objects->insert(pair<string, rgw_saloid_t>(hash_prefix, i->second));
  }
  vector<string> subdirs;
  r = list_subdirs(path, &subdirs);
  if (r < 0) return r;
  ...
  for (vector<string>::iterator i = subdirs.begin(); i != subdirs.end(); ++i) {
    ...
    if (next_object) {
      if (next_object->is_max()) continue;
      if (candidate < next_object_string.substr(0, candidate.size())) continue;
    }
...
int HashIndex::list_by_hash(const vector<string> &path, const rgw_saloid_t &end,
                            int max_count, rgw_saloid_t *next,
                            vector<rgw_saloid_t> *out) {
  ceph_assert(out);
...
int HashIndex::list_by_hash_bitwise(const vector<string> &path,
  ...
  if (next) *next = rgw_saloid_t::get_max();
  return 0;
}
