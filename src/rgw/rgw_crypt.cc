                           ...
                           std::map<std::string, std::string>& crypt_http_responses)
{
...

  {
    ...
    if (! req_sse.empty()) {
      ...
      set_attr(attrs, RGW_ATTR_CRYPT_KEYID, key_id);
      std::string actual_key;
      res = make_actual_key_from_sse_s3(s, attrs, s->penv.kms_cache.get(), y, actual_key);
      if (res != 0) {
        ldpp_dout(s, 5) << "ERROR: failed to retrieve actual key from key_id: " << key_id << dendl;
                           ...
                           bool copy_source)
{
  ...
  if (stored_mode == "AES256") {
    ...
    std::string key_id = get_str_attribute(attrs, RGW_ATTR_CRYPT_KEYID);
    std::string actual_key;
    res = reconstitute_actual_key_from_sse_s3(s, attrs, s->penv.kms_cache.get(), y, actual_key);
    if (res != 0) {
