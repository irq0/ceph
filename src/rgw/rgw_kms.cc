                            ...
                            std::string& actual_key)
{
  ...
  const auto fetch = [&](std::string &out_secret) {
    if (RGW_SSE_KMS_BACKEND_BARBICAN == kms_backend) {
      return get_actual_key_from_barbican(dpp, cct, key_id, out_secret);
    }

    if (RGW_SSE_KMS_BACKEND_VAULT == kms_backend) {
      return reconstitute_actual_key_from_vault(dpp, cct, kctx, attrs,
                                                actual_key);
    }
    ...
    if (RGW_SSE_KMS_BACKEND_TESTING == kms_backend) {
      ...
      return get_actual_key_from_conf(dpp, cct, key_id, key_selector,
                                      actual_key);
    }
