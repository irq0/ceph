...
http::status SFSStatusPage::render(std::ostream& os) {
  ...
  // db stats
  sqlite3_db_status(sqlite_db, SQLITE_DBSTATUS_CACHE_USED, &current, &highwater,
                    false);
  os << "<li> cache_used: " << current << " bytes (high: " << highwater
     << " bytes)</li>\n";
  sqlite3_db_status(sqlite_db, SQLITE_DBSTATUS_CACHE_USED_SHARED, &current,
                    &highwater, false);
  os << "<li> cache_used_shared: " << current << " bytes (high: " << highwater
     << " bytes)</li>\n";

  sqlite3_db_status(sqlite_db, SQLITE_DBSTATUS_CACHE_HIT, &current, &highwater,
                    false);
  os << "<li> cache_hit: " << current << " (high: " << highwater << ")</li>\n";
  sqlite3_db_status(sqlite_db, SQLITE_DBSTATUS_CACHE_MISS, &current, &highwater,
                    false);
