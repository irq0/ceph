...
PerfCounters* create_op_hist_counters(
    CephContext* cct, const std::string& name, RGWOpType type) {
  using axis = PerfHistogramCommon::axis_config_d;

  PerfCountersBuilder pcb(cct, name, l_rgw_op_hist_first, l_rgw_op_hist_last);
  pcb.set_prio_default(PerfCountersBuilder::PRIO_USEFUL);
  pcb.add_time_histogram(l_rgw_op_hist_lat, "op_lat",
      axis::web_latency("lat", 16), "Request latency distribution");
  PerfCounters* new_counters = pcb.create_perf_counters();
  cct->get_perfcounters_collection()->add(new_counters);
  return new_counters;
}
...
PerfCounters* get(CephContext* cct, RGWOpType type, const char* op_name) {
  ...
  if (!counters) {
    ...
    counters = slot.load(std::memory_order_relaxed);
    if (!counters) {
      const std::string key =
          ceph::perf_counters::key_create(rgw_op_hist_key, {{"op", op_name}});
      counters = create_op_hist_counters(cct, key, type);
