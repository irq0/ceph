...
class WebCache {
 ...
 public:
  ...
  void expire_erase(std::chrono::seconds ttl);
  ...
  FRIEND_TEST(WebCacheTest, SieveExample);
  FRIEND_TEST(WebCacheTest, ExpireEraseBasics);
  FRIEND_TEST(WebCacheTest, ExpireEraseHandleEmpty);
};
...
void WebCache<Key, Value>::sieve_expire_erase_unmutexed(
  ...
  const auto first_not_expired = std::find_if_not(
      sieve_queue.rbegin(), sieve_queue.rend(), [&](Node* node) {
        const auto age = now - node->added_ts;
        return age > ttl;  // expired
      });

  if (first_not_expired != sieve_queue.rend()) {
    out_expired.insert(
        out_expired.begin(), first_not_expired.base(), sieve_queue.end());
    sieve_queue.erase(first_not_expired.base(), sieve_queue.end());
  }
}

template <typename Key, typename Value>
void WebCache<Key, Value>::expire_erase(std::chrono::seconds ttl) {
  std::lock_guard<std::shared_mutex> lock(_cache_mutex);
  const auto now = ceph::real_clock::now();
