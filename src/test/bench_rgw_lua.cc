...
static rgw::lua::LuaCodeType maybe_compile_lua(rgw::sal::LuaManager* manager, bool do_compile, const std::string& script) {
  if (do_compile) {
    TestBackground b(manager);
    b.start();
    b.process_script_add(script);
    WAIT_FOR_BACKGROUND;
    std::vector<char> bytecode;
    ceph_assert(b.get_script_bytecode(script, bytecode) >= 0);
    return bytecode;
  }
...
void BM_RGWLua_Metadata(benchmark::State &state) {
  ...
  const rgw::lua::LuaCodeType script =
      maybe_compile_lua(pe.lua.manager.get(), state.range(0) == 1, R"(
...
BENCHMARK(BM_RGWLua_Hello)->Range(0, 1);
BENCHMARK(BM_RGWLua_Hello_NewState);
BENCHMARK(BM_RGWLua_Metadata)->Range(0, 1);
BENCHMARK(BM_RGWLua_WriteMetadata)->Range(0, 1);
BENCHMARK(BM_RGWLua_Acl)->Range(0, 1);
BENCHMARK(BM_RGWLua_NestedLoop)->Range(0, 1);
